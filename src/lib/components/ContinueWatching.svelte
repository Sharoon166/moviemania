...
{#if items.length > 0}
	<div class="space-y-4 px-4 md:px-8">
		<h2 class="font-display text-2xl font-bold text-fg">Continue Watching</h2>
		<div class="flex gap-3 overflow-x-auto pb-4" style="scrollbar-width: none;">
			{#each items as item (item.id)}
				...
				>
					...
					>
						<PlayIcon class="h-10 w-10 text-fg" weight="fill" />
					</div>
					...
					<button
						...
						class="absolute top-2 right-2 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-black/60 text-fg opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:opacity-100 hover:bg-fg/20"
					>
