...
<section class="mx-auto max-w-7xl px-4 pb-12 md:px-8">
	<h2 class="mb-5 font-display text-2xl font-bold text-fg">Cast</h2>
	<div
	...
	>
		{#each cast.slice(0, 15) as member (member.id)}
			<div class="flex w-30 shrink-0 snap-start flex-col items-center gap-2.5 text-center">
				<div class="overflow-hidden rounded-full ring-2 ring-fg/10">
				...
				<p class="w-full truncate text-sm font-medium text-fg">{member.name}</p>
